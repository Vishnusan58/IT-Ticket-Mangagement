...
| ID       | User Story                                                                                                     | Acceptance Criteria                                                                                                                           | Points | Assigned Dev |
| -------- | -------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------------ |
| **US1**  | As a User, I want to raise a ticket with title, description, and category so that support can help me.         | Ticket gets auto-ID; initial status is Raised and system transitions it to Open; requester is stored; creation is logged in history.          | 3      | Dev2         |
| **US2**  | As a System, I want to auto-assign the ticket to the least-loaded agent so workload stays balanced.            | Assignment happens when ticket becomes Open; only counts Open/In-Progress/Awaiting; ties resolved by lowest agentId; history logs assignment. | 5      | Dev3         |
| **US3**  | As a User/Agent/Admin, I want role-based ticket views with status/date filters so I see only relevant tickets. | Users see own tickets; agents see assigned; admins see all; filters by status/date work correctly.                                            | 5      | Dev4         |
| **US4**  | As a User, I want to edit my ticket description only when Open or Reopened.                                    | Edit is blocked in other states; history records the edit with timestamp.                                                                     | 3      | Dev2         |
| **US5**  | As a User, I want to close my ticket with confirmation or move it to Awaiting Response with a message.         | Close sets status to Resolved; Awaiting stores message and sets status Awaiting; history logs action.                                         | 5      | Dev2         |
| **US6**  | As an Agent/Admin, I want to add notes so updates are tracked.                                                 | Notes store author, timestamp, and text; visible to user and admin; logged in history.                                                        | 3      | Dev5         |
| **US7**  | As a User, I want to reopen a resolved ticket so unresolved issues continue.                                   | Reopen allowed only from Resolved; status becomes Reopened; ticket returns to assignment flow; history logged.                                | 2      | Dev2         |
| **US8**  | As a User, I want to rate a resolved ticket so service quality is measured.                                    | Rating allowed only when Resolved; rating < 2 flags agent; rating stored per ticket.                                                          | 5      | Dev6         |
| **US9**  | As an Admin, I want to view agent ratings and flags.                                                           | Report shows average rating and low-rating count per agent.                                                                                   | 3      | Dev6         |
| **US10** | As an Admin, I want to reassign tickets when category is wrong.                                                | Admin selects new agent/team; reassignment logged with reason.                                                                                | 3      | Dev3         |
| **US11** | As an Admin, I want monthly resolved vs reopened reports.                                                      | System shows count per month for resolved and reopened tickets.                                                                               | 5      | Dev6         |
| **US12** | As a System/Admin, I want escalation for tickets unresolved over 24 hours.                                     | Tickets older than 24h and not resolved appear in escalation list; admin can forward or reassign.                                             | 5      | Dev3         |
| **US13** | As an Admin, I want to manage change requests with approval, expiry, and archiving.                            | Only admin can approve/renew/remove; expiry enforced; archive after 1 year; expiring-soon list works.                                         | 8      | Dev7         |
| **US14** | As an Agent, I want to implement approved changes and log notes.                                               | Only approved changes can be implemented; note is stored; status updated; history logged.                                                     | 3      | Dev7         |
| **US15** | As a System, I want immutable ticket history for audit.                                                        | All state changes, notes, ratings, reassignments append history; no deletes/edits allowed.                                                    | 5      | Dev5         |
| **US16** | As an Admin, I want role-based access control.                                                                 | Only admin can change roles; unauthorized actions are blocked.                                                                                | 5      | Dev1         |
| **US17** | As a User/Admin, I want login and role simulation.                                                             | Login selects user and role; menus adapt to role.                                                                                             | 3      | Dev1         |
| **US18** | As a System, I want initial data seeding for demo.                                                             | Sample users, agents, tickets, and change requests exist on startup.                                                                          | 3      | Dev8         |
| **US19** | As a Team, we want integrated menus and flow for demo.                                                         | All modules work together in console; no crashes; smooth flow.                                                                                | 5      | Dev8         |
